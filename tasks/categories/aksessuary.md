...
- [x] Папка создана: `categories/aksessuary/`
- [x] `data/aksessuary_clean.json` создан
- [x] Keywords кластеризованы
- [x] `meta/aksessuary_meta.json` template
- [x] `content/aksessuary_ru.md` placeholder
- [x] `research/RESEARCH_DATA.md` template

**Init Validation:**

```bash
python3 -c "import json; json.load(open('categories/aksessuary/data/aksessuary_clean.json')); print('PASS')"
```
...
- [ ] Прочитать `data/aksessuary_clean.json`
...
- [ ] Записать в `meta/aksessuary_meta.json`
...
```bash
grep -c "^## Block" categories/aksessuary/research/RESEARCH_DATA.md
```
...
```bash
python3 scripts/validate_content.py categories/aksessuary/content/aksessuary_ru.md "{keyword}" --mode seo
```
