...
def process_category(category_path):
    ...
    for norm, group in clusters.items():
        ...
        if losers:
            change_msg = f"Kept '{winner['keyword']}' ({winner['volume']}). Dropped: " + ", ".join([f"'{l['keyword']}' ({l['volume']})" for l in losers])
    ...
    new_data['stats'] = {
        "before": data.get('stats', {}).get('before', 0), # Preserve original before count if possible, or use current total
        "after": total_count,
