...
-   [ ] **Задача 1.1** Реорганизация `tests/`
...
-   [ ] **Задача 1.2** Расширить `conftest.py`
...
-   [ ] **Задача 1.3** Создать `helpers/file_builders.py`
...
-   [ ] **Задача 1.4** Создать базовые фикстуры в `fixtures/`
    -   `fixtures/csv/valid_structure.csv` (минимальная валидная структура)
    -   `fixtures/json/valid_meta.json` (пример meta)
    -   `fixtures/md/valid_content_ru.md` (пример контента)

#### Week 2: Core Utils Coverage (приоритет 🔴)

-   [ ] **Задача 2.1** `test_seo_utils.py` — расширить до 80%+ coverage

    -   Тесты на `clean_markdown` (edge cases)
    -   Тесты на `normalize_text` (спецсимволы, эмодзи)
    -   Тесты на `count_words` (вся логика)
    -   Тесты на `parse_front_matter` (валидный/инвалидный YAML)
    -   Тесты на `slugify` (кириллица, украинский, edge cases)
