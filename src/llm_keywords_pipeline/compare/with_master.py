...
def generate_report(cat_data: dict, master_data: dict) -> str:
    ...
    if only_in_cat:
        ...
        for key in sorted(only_in_cat):
            for rec in cat_data[key]:
                lines.append(
                    f"| {rec['keyword']} | {rec['volume']} | {rec['category']} | {rec['source_type']} |"
                )
    else:
    ...
    if only_in_master:
        ...
        for key in sorted(only_in_master):
            for rec in master_data[key]:
                lines.append(
                    f"| {rec['keyword']} | {rec['volume']} | {rec['category']} | {rec['type']} |"
                )
    else:
        lines.append("Нет расхождений.")
    lines.append("")
    ...
    if volume_diffs:
        ...
        for d in sorted(volume_diffs, key=lambda x: abs(x["diff"]), reverse=True):
            ...
            lines.append(
                f"| {d['keyword']} | {d['category']} | {d['vol_cat']} | {d['vol_master']} | {sign}{d['diff']} |"
            )
    else:
        lines.append("Нет расхождений.")
    lines.append("")
