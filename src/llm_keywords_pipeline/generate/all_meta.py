...
def generate_meta_ru(slug: str, data: dict) -> dict:
    ...
    if types_str:
        description = (
            f"{name} от производителя Ultimate. {capitalize_first(types_str)}. Опт и розница."
        )
    else:
...
def generate_meta_uk(slug: str, data: dict) -> dict:
    ...
    if types_str:
        description = (
            f"{name} від виробника Ultimate. {capitalize_first(types_str)}. Опт і роздріб."
        )
    else:
...
def process_categories(
    lang: str = "ru", dry_run: bool = False, force: bool = False
) -> tuple[int, int, int]:
    ...
    for category_dir in sorted(categories_dir.iterdir()):
        ...
        if existing_meta and not force:
            ...
            has_manual_data = any(
                k in existing_meta for k in ["types", "volumes", "keywords_in_content"]
            )
            if has_manual_data:
