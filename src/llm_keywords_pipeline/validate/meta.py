...
from llm_keywords_pipeline.core.keywords import MorphAnalyzer, keyword_matches_text
...
def get_primary_keywords(keywords_data: dict[str, Any]) -> list[str]:
    ...
    if "keywords" in keywords_data:
        ...
        if isinstance(kw_section, list) and kw_section:
            ...
            max_item = max(
                kw_section, key=lambda x: x.get("volume", 0) if isinstance(x, dict) else 0
            )
...
def validate_title(
    title: str, primary_keywords: list[str] | None = None, lang: str = "ru"
) -> dict[str, Any]:
    """
    Validate meta title.
    ...
    if primary_keywords:
        from llm_keywords_pipeline.core.keywords import MorphAnalyzer
...
def validate_description(
    ...
    if producer_found:
        producer_msg = (
            "OK (contains 'від виробника')" if lang == "uk" else "OK (contains 'от производителя')"
        )
        results["checks"]["producer"]["passed"] = True
        results["checks"]["producer"]["message"] = producer_msg
    else:
        missing_msg = (
            "Missing 'від виробника Ultimate'"
            if lang == "uk"
            else "Missing 'от производителя Ultimate'"
        )
        results["checks"]["producer"]["message"] = missing_msg

    # Check if this is a "shop" pattern (no Ultimate products) vs "producer" pattern
    shop_pattern = (
        r"в інтернет-магазині ultimate" if lang == "uk" else r"в интернет-магазине ultimate"
    )
    ...
    if wholesale_found:
        wholesale_msg = (
            "OK (contains 'опт/роздріб')" if lang == "uk" else "OK (contains 'опт/розница')"
        )
...
def validate_meta_file(
    meta_path: str, keywords_path: str | None = None, lang: str = "ru"
) -> dict[str, Any]:
    """
    ...
    if (
        results["title"]["overall"] == "PASS"
        and results["description"]["overall"] == "PASS"
        and h1_ok
    ):
        results["overall"] = "PASS"
        ...
        if not h1_ok:
            ...
            results["warnings"] = results.get("warnings", []) + [
                f"H1 validation: {h1_validation['message']}"
            ]
        else:
            results["overall"] = "WARNING"
    else:
