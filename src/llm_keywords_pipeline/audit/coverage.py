...
Usage:
    uv run python -m llm_keywords_pipeline.audit.coverage --slug aktivnaya-pena --lang uk --verbose
    uv run python -m llm_keywords_pipeline.audit.coverage --lang uk  # batch all UK categories
    uv run python -m llm_keywords_pipeline.audit.coverage --slug X --lang uk --json
"""
...
from llm_keywords_pipeline.core.config import PROJECT_ROOT
from llm_keywords_pipeline.core.coverage import PreparedText, audit_category, check_keyword
...
def audit_category_coverage(slug: str, lang: str = "uk") -> dict:
    """
    Audit a single category's keyword coverage.
    ...
    Returns:
        Coverage report dict with keys:
        - coverage_percent: float
        - covered: int
        - total: int
        - results: list of keyword results
    """
    data = load_category_data(slug, lang)
    if data is None:
        return {"error": f"Category {slug} ({lang}) not found", "coverage_percent": 0}
