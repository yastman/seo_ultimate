#!/usr/bin/env python3
"""
audit_unused_keywords.py — Аудит неиспользуемых RU ключей из ull_all_rus_keys.csv.

Usage:
    uv run python -m llm_keywords_pipeline.audit.unused
"""
...
CSV_FILE = PROJECT_ROOT / "ull_all_rus_keys.csv"
...
CATEGORY_PATTERNS = [
    # Главная страница
    (
        r"^автохимия$|^автокосметика$|интернет.?магазин|киев|украина|^детейлинг.?магазин|^детейлинг.?шоп|автохимия.?купить$|автокосметика.?купить$|для.?детейлинга$|автохимия.?и.?автокосметика|автокосметика.?профессиональная|профессиональная.?автохимия|автохимия.?каталог|автокосметика.?цен|автохимия.?цен",
        "glavnaya",
    ),
    # Мойка
    (r"пена|бесконтакт", "aktivnaya-pena"),
    (
        r"шампунь.*(ручн|кислот)|автошампунь.*(ручн|кислот)|(ручн|кислот).*шампунь",
    ...
    # Интерьер
    (
        r"химчист|салон.*чист|чист.*салон|ткан.*чист|чист.*ткан|потолок|велюр|алькантар",
        "sredstva-dlya-khimchistki-salona",
    ),
    (r"пятн|выводит", "pyatnovyvoditeli"),
    (r"запах|нейтрализ|поглотит|одорант", "neytralizatory-zapakha"),
    (
        r"пластик.*салон|торпед|полироль.*пластик|пластик.*полироль|панел.*прибор",
