    ...
    with open(file_path, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            if ':' in stripped_cell and (stripped_cell.startswith('L') or stripped_cell.startswith('SEO')):
                ...
                cat_name = parts[1].strip().split(',')[0] # Handle case where comma might be in line? No CSV reader handles commas.
...
def main():
    ...
    MANUAL_MAP = {
        ...
        'sredstva-dlya-kozhi': ['средства для кожи', 'уход за кожей'], # Mapping multiple CSV cats to one folder? Or vice versa.
