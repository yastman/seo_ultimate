...
def load_stopwords(lang='ru'):
    ...
    if stopwords_file.exists():
        ...
        try:
            ...
            if lang == 'uk':
                return {
                    'і', 'в', 'не', 'на', 'з', 'що', 'це', 'як', 'а', 'по',
                    'для', 'від', 'із', 'до', 'або', 'же', 'про', 'але', 'за', 'у',
                    'який', 'також', 'більше', 'бути', 'може', 'дуже'
                }
            else:
                return {
                    'и', 'в', 'не', 'на', 'с', 'что', 'это', 'как', 'а', 'по',
                    'для', 'от', 'из', 'к', 'или', 'же', 'о', 'но', 'за', 'у',
                    'который', 'также', 'более', 'быть', 'может', 'очень'
                }

# Дополнительные стоп-слова (вода) из анализа Адвего
ADDITIONAL_STOP_WORDS = {
    'особенно', 'актуально', 'важно', 'помнить', 'подход',
    'снижает', 'риск', 'позволяет', 'является', 'данный',
    'следует', 'необходимо', 'можно', 'нужно', 'различный',
    'который', 'такой', 'этот', 'свой', 'весь', 'один',
    'другой', 'наш', 'ваш', 'тот', 'сам', 'мочь', 'сказать'
}
...
def calculate_metrics_from_text(text: str, lang: str = 'ru') -> dict:
    ...
    russian_tokens = [
        ...
        if re.match(r'[а-яё]+', token.text.lower(), re.UNICODE)
    ]
    ...
    significant_lemma_counts = {lemma: count for lemma, count in lemma_counts.items()
    ...
    significant_lemmas = {lemma: count for lemma, count in lemma_counts.items()
...
def check_water(file_path, target_min=40, target_max=60):
    ...
    if metrics['most_common_significant']:
        print(f"   Самое частое значимое слово: '{metrics['most_common_significant']}' ({metrics['max_freq_significant']} раз)")
        print(f"   Значимых слов (без стоп-слов, freq>1): {metrics['total_significant']}")
    else:
        print(f"   Нет повторяющихся значимых слов")
    ...
    print(f"   (вспомогательная метрика для внутреннего анализа)")
