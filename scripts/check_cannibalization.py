...
def find_partial_intersections(keyword_map: Dict[str, List[Dict]]) -> List[Dict]:
    ...
    for i in range(len(keys)):
        short_kw = keys[i]
        short_cats = set(x['category'] for x in keyword_map[short_kw])
        ...
        for j in range(i + 1, len(keys)):
            ...
            # Found containment
            long_cats = set(x['category'] for x in keyword_map[long_kw])
            ...
            diff_cats = short_cats.symmetric_difference(long_cats)
            common_cats = short_cats.intersection(long_cats)
    ...
    for lang, path in [("RU", ru_path), ("UA", ua_path)]:
        ...
        if partials:
            ...
            for (c1, c2), issues in sorted_pairs:
                if len(issues) < 1: continue # filter noise?
