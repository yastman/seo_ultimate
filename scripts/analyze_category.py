...
def load_keywords_for_slug(slug: str, lang: str = "ru") -> Tuple[List[Dict], str, Optional[Dict]]:
    """
...
try:
    from scripts.seo_utils import L3_TO_SLUG, SLUG_TO_L3, get_l3_slug, get_commercial_modifiers
except ImportError:
    try:
        from seo_utils import L3_TO_SLUG, SLUG_TO_L3, get_l3_slug, get_commercial_modifiers
    except ImportError:
        ...
        _COMMERCIAL_MODIFIERS_FALLBACK = [
            'купить', 'цена', 'стоимость', 'заказать',
            'в наличии', 'доставка', 'недорого', 'дёшево', 'дешево',
            'интернет-магазин', 'магазин', 'каталог'
        ]
        ...
        get_commercial_modifiers = lambda lang="ru": (
            _COMMERCIAL_MODIFIERS_UK_FALLBACK if lang == "uk" else _COMMERCIAL_MODIFIERS_FALLBACK
        )
...
def generate_content_guidelines(analysis: Dict) -> Dict:
    ...
    if depth == "shallow":
        ...
        structure["faq"] = {
            "required": True,
            "count": "2-3",
            "note": "Практические вопросы"
        }
        structure["comparison_table"] = {"required": False}
        structure["types_section"] = {
            "required": True if count > 10 else False,
        ...
        structure["comparison_table"] = {
            "required": True,
            "note": "Сравнение типов/характеристик"
        }
        structure["types_section"] = {
            "required": True,
            "note": "Виды товаров с описанием"
        }
...
def list_all_categories():
    ...
    for r in results:
        ...
        print(f"{r['slug']:<25} {r['count']:>8} {r['volume']:>7} {r['depth']:<10} {r['format']:<12} {r['words']:<10} {cluster}")
...
def main():
    ...
    if output_json:
        ...
        source_label = {"clean_json": "Clean JSON (clustered)", "raw_json": "Raw JSON", "csv": "CSV fallback"}
