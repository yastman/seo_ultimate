...
description: >-
  ...
  подготовь промпт для ресёрча, research prompt, Perplexity Deep Research.
---
...
**Задача:** На основе данных ниже провести web research и собрать факты для написания SEO-текста категории.

**Как работать:**
1. Вставь содержимое этого файла в Perplexity → Deep Research.
2. Собери информацию по Разделу B (рынок/практика), **не придумывая** данные про наш ассортимент.
3. Верни результат в формате, готовом для вставки в `RESEARCH_DATA.md` (источники → факты → заметки).
4. Добавь краткую оценку качества/уверенности: что подтверждено хорошо, а что спорно или зависит от условий.

**Требование:** Любая цифра/утверждение — только с URL + цитатой/таймкодом. Если данных нет — так и написать.
...
**Задача:** Проведи глубокий web research по теме **{Название категории} ({EN название})**. Изучи профессиональные детейлинг-форумы, YouTube обзоры (с таймкодами), техническую документацию производителей. Сравни источники — если расходятся, дай диапазон и укажи от чего зависит.

**Контекст:** Пишу SEO-текст для страницы категории интернет-магазина автохимии. Ассортимент магазина уже известен (см. Раздел A выше). Нужны **общие знания о категории** для экспертного контента.

#### Что делать с данными из Раздела A
1. Считать Раздел A “истиной” про наш ассортимент: **не добавлять** в выводы “что у нас есть”, если этого нет в Разделе A.
2. Если в источниках встречаются дополнительные формы/типы (типично для рынка) — можно описать их **как рынок**, но пометить “не факт, что есть в нашем каталоге”.
3. Цель: собрать факты и практику применения, чтобы из них потом собрать текст категории (гайд, FAQ, таблицы выбора).

#### Как провести исследование (web research)
1. Найти 5–12 надёжных источников по теме (с приоритетом TDS/SDS/проф. гайды; форумы/YouTube — как доп.).
2. Снять противоречия: если источники расходятся — дать **диапазон** и факторы, от чего зависит.
3. Извлечь цифры/параметры: расход, время высыхания/впитывания, стойкость, температурные ограничения, типичные ошибки и последствия.

#### Выходной формат (чтобы я мог вставить в RESEARCH_DATA.md)
Сделай ответ в 3 блоках и в таком порядке:
1. `Sources (raw list)` — таблица: URL | что подтверждает | цитата/таймкод
2. `Extracted facts (ready for content)` — таблица: тезис | цифры/диапазон | условия/допущения | источник
3. `Notes` — список: что не удалось надёжно подтвердить / где источники спорят
