...
### Что такое `{category_title}` (приоритет над primary_keyword)

**Опциональное поле** в `_clean.json` для составных категорий или категорий с двумя ВЧ-ключами.

```json
{
  "id": "gubki-i-varezhki",
  "category_title": "Губки и варежки",  // ← приоритет!
  "keywords": [...]
}
```

**Когда использовать `category_title`:**
1. **Составная категория** — название содержит "и" (Щётки и кисти, Губки и варежки)
2. **Два сильных ВЧ-ключа** — нужно охватить оба (Автохимия и автокосметика)

**Логика выбора:**
```
ЕСЛИ category_title существует:
  использовать category_title для Title/H1/Description
ИНАЧЕ:
  использовать primary_keyword (MAX volume)
```

### Что такое `{primary_keyword}`

Это ключ с **максимальным volume** в категории. Используется когда `category_title` не задан.
...
**КРИТИЧНО:** Title и H1 используют **множественное число**, т.к. категория магазина содержит много товаров.

```
title_phrase = category_title ?? primary_keyword
H1 = phrase_to_plural(title_phrase)   # множественное первого слова
Title = H1 + " — купить..."           # тот же H1
```
...
H1: "Очистители дисков"      ✅ (множественное)
Title: "Очистители дисков — купить, цены | Ultimate"  ✅

НЕ ПРАВИЛЬНО:
H1: "Очистители для дисков"          ❌ (другая фраза!)
```

---

## 🚨 IRON RULE: keyword — ДОСЛОВНО (уточнено)

**`{category_title}` или `{primary_keyword}` из `_clean.json` используется в Title/H1/Description без изменения слов и порядка.**

**Дозволено ТОЛЬКО:**
1. Капитализация первой буквы
2. Конвертация первого слова в множественное (phrase_to_plural)

**НЕ ДОЗВОЛЕНО:**
- Сокращать фразу ("дисков" → "для дисков")
- Менять порядок слов
...
**Приоритет:** `category_title` > `primary_keyword`

```
_clean.json: "keywords": [{"keyword": "воск для авто", "volume": 1000}]

✅ Title: Воски для авто — купить, цены | Ultimate
✅ H1: Воски для авто
...
### Выбор ключевой фразы для Title
...
```
ЕСЛИ {title_phrase} ≤ 20 chars:
  {title_phrase} — купить в интернет-магазине Ultimate

ИНАЧЕ:
  {title_phrase} — купить, цены | Ultimate
```

**Примеры с category_title:**

| category_title | Title |
|----------------|-------|
| Автохимия и автокосметика | Автохимия и автокосметика — купить, цены \| Ultimate |
| Губки и варежки | Губки и варежки — купить в интернет-магазине Ultimate |
| Щётки и кисти для детейлинга | Щётки и кисти для детейлинга — купить, цены \| Ultimate |

**Примеры с primary_keyword (когда category_title не задан):**
...
**Формула:** `{title_phrase}` (category_title ?? primary_keyword) → **конвертировать в множественное**
...
1. **Прочитать** `categories/{slug}/data/{slug}_clean.json`
   - Проверить наличие `category_title` (приоритет!)
   - Если нет — определить `{primary_keyword}` (MAX volume)
   - `{title_phrase}` = `category_title` ?? `primary_keyword`
...
**Changelog v17.2:**
- 🔧 **Чёткая формула**: Title = H1 = phrase_to_plural(title_phrase)
- 📋 **IRON RULE уточнено**: дозволено только капитализация + множественное
- ❌ **Запрещено**: сокращать или изменять фразу

**Changelog v17.0:**
- 🆕 **category_title**: новое опциональное поле для составных категорий и двух ВЧ-ключей
- 📋 **Приоритет**: `category_title` > `primary_keyword` в Title/H1/Description
- 🎯 **Примеры**: "Губки и варежки", "Автохимия и автокосметика", "Щётки и кисти для детейлинга"
