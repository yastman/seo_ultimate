...
```
{keywords[0]} или {name из _meta.json}
```

- Множественное число (категория = список товаров)
- БЕЗ "Купить" — это в title, не в H1
- Пример: "Шампуни для ручной мойки" (не "Купить шампунь...")

### H2 (2-4 на страницу)

| H2 | Источник ключа |
|----|----------------|
| "Как выбрать: сценарии" | `keywords[1]` или общий |
| "На что смотреть при выборе" | `keywords[2]` или общий |
| "Чего избегать при выборе" | Без ключа (опционально) |
| "FAQ" | Без ключа |

**Правило:** НЕ вставлять primary keyword в каждый H2. Достаточно 1 раз в H2.
...
- Каждый `micro_intent` из `_clean.json` = отдельный H3
- Формулировка вопросом: "Какой шампунь лучше для керамики?"
- НЕ "Шампунь для керамики" — это не вопрос

### Сводная таблица

| Источник | Где использовать | Частота |
|----------|------------------|---------|
| `keywords[0]` (primary) | H1, intro, 1× в тексте | 2-3 |
| `keywords[1-2]` | 1× в H2, первый абзац | 1 каждый |
| `keywords[3+]` | Таблицы, текст | 1 каждый |
| `synonyms` (без meta_only) | По тексту, таблицы | 1-2 |
| `entities` | Таблицы, FAQ-ответы | 3-4 |
| `micro_intents` | **H3 в FAQ** | все |

### Пример структуры

```markdown
# Шампуни для ручной мойки              ← keywords[0]

{Intro с keywords[0]}

## Как выбрать: сценарии                ← keywords[1] в тексте под H2

| ... таблица с keywords[3+], entities ...

**Если X** → Y                          ← synonyms

## На что смотреть при выборе           ← keywords[2] в тексте под H2

## FAQ

### Какой шампунь для ручной мойки лучше?   ← micro_intent[0]
{Ответ с entities}

### Чем отличается от бесконтактного?       ← micro_intent[1]
{Ответ}
```
...
- Секция "Как применять/наносить/работать" → это how-to (заменить на "Что влияет на результат")
- Секция "Товар + аксессуар: комбинации" → это cross-sell
- Секция "Ошибки при работе" → это инструкция (заменить на "Чего избегать при выборе")
