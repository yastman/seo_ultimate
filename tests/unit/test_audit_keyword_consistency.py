...
class TestScanActualKeywords:
    ...
    def test_returns_empty_dict_when_no_files(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("scripts.audit_keyword_consistency.CATEGORIES_DIR", str(tmp_path))
    ...
    def test_extracts_keywords_from_list_format(self, tmp_path, monkeypatch):
        ...
        (cat_path / "test-slug_clean.json").write_text(json.dumps(data, ensure_ascii=False), encoding="utf-8")
    ...
    def test_extracts_keywords_from_dict_format(self, tmp_path, monkeypatch):
        ...
        (cat_path / "test-slug2_clean.json").write_text(json.dumps(data, ensure_ascii=False), encoding="utf-8")
    ...
    def test_handles_empty_keywords(self, tmp_path, monkeypatch):
        ...
        (cat_path / "empty-slug_clean.json").write_text(json.dumps(data, ensure_ascii=False), encoding="utf-8")
    ...
    def test_handles_slug_field_fallback(self, tmp_path, monkeypatch):
        ...
        (cat_path / "fallback-slug_clean.json").write_text(json.dumps(data, ensure_ascii=False), encoding="utf-8")
