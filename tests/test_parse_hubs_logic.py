...
from parse_hubs_logic import slugify, parse_structure, generate_hub_tasks

class TestParseHubsLogic:
    def test_slugify(self):
        assert slugify("Тест") == "test"
        assert slugify("Тест Пробел") == "test-probel"
        assert slugify("L1: Категория") == "l1-kategoriya" # colons are replaced by dash in map but slugify iterates char by char
        # ':' is in map as '-'
        # ' ' is '-'
