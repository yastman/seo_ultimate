...
| `fix` | Repair utilities for duplicate, missing, misplaced, and legacy data. | Some modules are legacy/internal support tools. |
| `sync` | Synchronizes keyword and semantic data across files. | Useful internally; public use requires compatible data layout. |
...
## Stable vs Legacy Surface

The strongest public surface is the validation/audit/core code and its tests. Some
generation, repair, sync, and migration modules were originally built for a private
project layout and remain in the public version to show the broader pipeline shape.
...
suite focus on the parts that can be run without private datasets.
...
package can be linted and tested without access to private inputs.
