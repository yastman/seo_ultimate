...
1. **E2E integration test** — полный workflow
2. **Direct import вместо subprocess** — quality_runner.py
3. **Error handling** — try/except везде
4. **Caching** — HTTP requests, Natasha

### P3: Средние

1. **Gradual validation** — только изменённые файлы
2. **Analytics dashboard** — progress по категориям

### P4: Низкие

1. **Архивировать legacy** — check_lsi_metrics.sh, url_filters.py
2. **Docker image** — reproducible environment
